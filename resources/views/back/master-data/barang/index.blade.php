@extends('layouts.pos.app')

@section('content')
<header class="container-fluid pt-3">
    <h1 class="color-primary header1 mb-3 font-weight-700">List Barang</h1>
</header>

<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            <div class="table-container mb-12">
                <div class="table-responsive">
                    <table class="table" id="table">
                        <thead>
                            <tr>
                                <th>No.</th>
                                <th>Kode</th>
                                <th>Nama</th>
                                <th>Stok</th>
                                <th>Satuan</th>
                                <th>Satuan 2</th>
                                <th>Isi Satuan 2</th>
                                <th>Satuan 3</th>
                                <th>Isi Satuan 3</th>
                                <th>Supplier</th>
                                <th>Harga Beli</th>
                                <th>Harga Jual</th>
                                <th>Harga Jual 1</th>
                                <th>Harga Jual 2</th>
                                <th>Harga Jual 3</th>
                                <th>Diskon Jual</th>
                                <th>Diskon Pembelian (1)</th>
                                <th>Jumlah Diskon (%)</th>
                                <th>Diskon Pembelian (2)</th>
                                <th>Jumlah Diskon (%)</th>
                                <th>Diskon Pembelian (3)</th>
                                <th>Jumlah Diskon (%)</th>
                                <th>Diskon Pembelian (4)</th>
                                <th>Jumlah Diskon (%)</th>
                                <th>Berat</th>
                                <th>Omset</th>
                                <th>Size</th>
                                <th>Deskripsi</th>
                                <th>Aksi</th>
                            </tr>
                        </thead>
                        <tbody></tbody>
                    </table>
                </div>
            </div>
        </div>

        <div class="col-lg-12 mb-2">
            <hr class="divider">
            <div class="d-flex justify-content-between">
                <div class="input-group form-group mr-5 search-button-input">
                    <input type="text" class="form-control form-control-sm border-right-0" placeholder="Cari Grup" aria-label="Recipient's username" aria-describedby="button-addon2">
                    <div class="input-group-append">
                        <button class="btn btn-custom search-button px-3" type="button" id="button-addon2">
                            <i class="fas fa-search"></i>
                        </button>
                    </div>
                </div>
                <div>
                    <button class="btn btn-custom blue px-3 ml-3">
                        <i class="fas fa-redo mr-2"></i>
                        Refresh
                    </button>
                    <a class="text-decoration-none" href="{{ route('back.home.index') }}">
                        <button class="btn btn-custom red px-3 ml-3">
                            <i class="fas fa-times-circle mr-2"></i>
                            Keluar
                        </button>
                    </a>
                    {{-- <button class="btn btn-custom red px-3 ml-3">
                        <i class="fas fa-eraser mr-2"></i>
                        Hapus Baris
                    </button> --}}
                    <a class="text-decoration-none" href="{{ route('back.master-data.barang.export') }}">
                        <button class="btn btn-custom green px-3 ml-3">
                            <i class="fas fa-file-excel mr-2"></i>
                            File Excel
                        </button>
                    </a>
                    <a class="text-decoration-none" href="{{ route('back.master-data.barang.print-barcode') }}" target="_blank">
                        <button class="btn btn-custom green px-3 ml-3">
                            <i class="fas fa-barcode mr-2"></i>
                            Barcode
                        </button>
                    </a>
                    {{-- <button class="btn btn-custom green px-3 ml-3">
                        <i class="fas fa-print mr-2"></i>
                        Print
                    </button> --}}
                </div>
            </div>
        </div>
    </div>
</div>

{{-- Modal --}}
<div class="modal fade" id="modalPage" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="modalPageLabel" aria-hidden="true">
    <div class="modal-dialog modal-md">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="modal-title"></h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body" id="modal-body">
                ...
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Tutup</button>
                <button type="button" class="btn btn-primary" id="btn-save">Simpan</button>
            </div>
        </div>
    </div>
</div>
@endsection

@push('css')
<link rel="stylesheet" href="{{ asset('back_assets/dist/css/pages/master-data/barang/index.css') }}">
@endpush

@push('js')
<script src="{{ asset('back_assets/dist/js/script.js') }}"></script>
<script>
    var table;
    table = $('#table').DataTable({
        "initComplete": function(settings, json) {
            $('#table_filter').remove();
            $('#table_length').remove();
            $('#table_paginate').remove();
        },
        "responsive": false,
        "pagingType": "simple_numbers",
        "paging": true,
        "autoWidth": true,
        "scrollX": true,
        "scrollCollapse": true,
        "columnDefs": [
            { "width": '50px', "targets": 0 },
            { "width": '100px', "targets": 1 },
            { "width": '150px', "targets": 2 },
            { "width": '75px', "targets": 3 },
            { "width": '100px', "targets": 4 },
            { "width": '100px', "targets": 5 },
            { "width": '150px', "targets": 6 },
            { "width": '75px', "targets": 7 },
            { "width": '100px', "targets": 8 },
            { "width": '150px', "targets": 9 },
            { "width": '100px', "targets": 10 },
            { "width": '100px', "targets": 11 },
            { "width": '125px', "targets": 12 },
            { "width": '125px', "targets": 13 },
            { "width": '125px', "targets": 14 },
            { "width": '75px', "targets": 15 },
            { "width": '75px', "targets": 16 },
            { "width": '125px', "targets": 17 },
            { "width": '75px', "targets": 18 },
            { "width": '125px', "targets": 19 },
            { "width": '75px', "targets": 20 },
            { "width": '125px', "targets": 21 },
            { "width": '75px', "targets": 22 },
            { "width": '125px', "targets": 23 },
            { "width": '125px', "targets": 24 },
            { "width": '75px', "targets": 25 },
            { "width": '125px', "targets": 26 },
            { "width": '200px', "targets": 27 },
        ],
        "fixedColumns": true,
        'pageLength': 10,
        "bInfo" : false,
        "lengthMenu": [10, 25, 50, 100],
        "processing": true,
        "language": {
            "processing": '<div class="d-flex justify-content-center"><div class="spinner-border" role="status"><span class="sr-only">Loading...</span></div></div>'
        },
        "serverSide": true,
        "searching": false,
        "ajax": {
            "headers": {'X-CSRF-TOKEN': '{{ csrf_token() }}'},
            "url": "{{ route('api.back.master-data.barang') }}",
            "dataType": "json",
            "type": "POST"
        },
        "columns": [
            {
                "data": "no",
                "className": "align-middle"
            },
            {
                "data": "kode",
                "className": "align-middle"
            },
            {
                "data": "nama",
                "className": "align-middle"
            },
            {
                "data": "stok",
                "className": "align-middle"
            },
            {
                "data": "satuan_1",
                "className": "align-middle"
            },
            {
                "data": "satuan_2",
                "className": "align-middle"
            },
            {
                "data": "isi_satuan_2",
                "className": "align-middle"
            },
            {
                "data": "satuan_3",
                "className": "align-middle"
            },
            {
                "data": "isi_satuan_3",
                "className": "align-middle"
            },
            {
                "data": "supplier",
                "className": "align-middle"
            },
            {
                "data": "harga_beli",
                "className": "align-middle"
            },
            {
                "data": "harga_jual",
                "className": "align-middle"
            },
            {
                "data": "harga_jual_1",
                "className": "align-middle"
            },
            {
                "data": "harga_jual_2",
                "className": "align-middle"
            },
            {
                "data": "harga_jual_3",
                "className": "align-middle"
            },
            {
                "data": "diskon_jual",
                "className": "align-middle"
            },
            {
                "data": "diskon_qty_1",
                "className": "align-middle"
            },
            {
                "data": "diskon_amount_1",
                "className": "align-middle"
            },
            {
                "data": "diskon_qty_2",
                "className": "align-middle"
            },
            {
                "data": "diskon_amount_2",
                "className": "align-middle"
            },
            {
                "data": "diskon_qty_3",
                "className": "align-middle"
            },
            {
                "data": "diskon_amount_3",
                "className": "align-middle"
            },
            {
                "data": "diskon_qty_4",
                "className": "align-middle"
            },
            {
                "data": "diskon_amount_4",
                "className": "align-middle"
            },
            {
                "data": "berat",
                "className": "align-middle"
            },
            {
                "data": "omset",
                "className": "align-middle"
            },
            {
                "data": "size",
                "className": "align-middle"
            },
            {
                "data": "deskripsi",
                "className": "align-middle"
            },
            {
                "data": "actions",
                "orderable": false,
                "className": "text-center align-middle"
            }
        ]
    })

    $('body').on('click', '.btn-page', function () {
        event.preventDefault()
        let url = $(this).attr('href')
        let title = $(this).data('title')

        $('#modalPage').modal('show')
        $.ajax({
            url: url,
            method: 'GET',
            dataType: 'html',
            success: function (html) {
                $('#modal-title').text(title)
                $('#modal-body').html(html)
            }
        })
    })

    $('body').on('click', '#btn-save', function () {
        $('#form-validation').submit()
    })
</script>
@endpush
